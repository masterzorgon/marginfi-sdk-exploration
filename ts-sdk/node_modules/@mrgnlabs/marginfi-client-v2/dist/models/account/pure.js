...
class MarginfiAccount {
    ...
    async makeDepositIx(program, banks, amount, bankAddress) {
        ...
        const depositIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) ? this.wrapInstructionForWSol(ix, amount) : [ix];
        return {
            instructions: depositIxs,
            keys: [],
        };
    }
    async makeRepayIx(program, banks, amount, bankAddress, repayAll = false, opt = {}) {
        const bank = banks.get(bankAddress.toBase58());
        if (!bank)
            throw Error(`Bank ${bankAddress.toBase58()} not found`);
        ...
        const repayIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) && wrapAndUnwrapSol ? this.wrapInstructionForWSol(ix, amount) : [ix];
        ixs.push(...repayIxs);
        return {
            instructions: ixs,
            keys: [],
    ...
    async makeWithdrawIx(program, banks, amount, bankAddress, withdrawAll = false, opt = {}) {
        ...
        const withdrawIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) && wrapAndUnwrapSol ? this.wrapInstructionForWSol(ix) : [ix];
    ...
    async makeBorrowIx(program, banks, amount, bankAddress, opt) {
        ...
        const borrowIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) ? this.wrapInstructionForWSol(ix) : [ix];
        ixs.push(...borrowIxs);
        return {
            instructions: ixs,
            keys: [],
