...
class MarginfiClient {
    ...
    // Factories
    // --------------------------------------------------------------------------
    constructor(config, program, wallet, isReadOnly, group, banks, priceInfos, mintDatas, feedIdMap, addressLookupTables, preloadedBankAddresses, bankMetadataMap, sendEndpoint, spamSendTx = true, skipPreflightInSpam = true) {
        this.config = config;
    ...
    static async fetch(config, wallet, connection, clientOptions) {
        ...
        const program = new anchor_1.Program(_1.MARGINFI_IDL, provider);
        ...
        const { marginfiGroup, banks, priceInfos, tokenDatas, feedIdMap } = await MarginfiClient.fetchGroupData(program, config.groupPk, connection.commitment, preloadedBankAddresses, bankMetadataMap);
        ...
        return new MarginfiClient(config, program, wallet, readOnly, marginfiGroup, banks, priceInfos, tokenDatas, feedIdMap, addressLookupTables, preloadedBankAddresses, bankMetadataMap, sendEndpoint, spamSendTx, skipPreflightInSpam);
    }
    ...
    static async fetchGroupData(program, groupAddress, commitment, bankAddresses, bankMetadataMap) {
        ...
        const feedIdMap = await (0, utils_1.buildFeedIdMap)(bankDatasKeyed.map((b) => b.data.config), program.provider.connection);
        // const oracleKeys = bankDatasKeyed.map((b) => b.data.config.oracleKeys[0]);
        const mintKeys = bankDatasKeyed.map((b) => b.data.mint);
        const emissionMintKeys = bankDatasKeyed.map((b) => b.data.emissionsMint).filter((pk) => !pk.equals(web3_js_1.PublicKey.default));
        const oracleKeys = bankDatasKeyed.map((b) => (0, utils_1.findOracleKey)(_1.BankConfig.fromAccountParsed(b.data.config), feedIdMap));
        // Batch-fetch the group account and all the oracle accounts as per the banks retrieved above
        const allAis = await (0, mrgn_common_1.chunkedGetRawMultipleAccountInfoOrdered)(program.provider.connection, [groupAddress.toBase58(), ...oracleKeys.map((pk) => pk.toBase58()), ...mintKeys.map((pk) => pk.toBase58()), ...emissionMintKeys.map((pk) => pk.toBase58())]); // NOTE: This will break if/when we start having more than 1 oracle key per bank
        ...
        const banks = new Map(bankDatasKeyed.map(({ address, data }) => {
            ...
            const bank = _1.Bank.fromAccountParsed(address, data, feedIdMap, bankMetadata);
        ...
        return (await this.program.provider.connection.getProgramAccounts(this.programId, {
            ...
            filters: [
                ...
                },
                {
                    memcmp: {
                        offset: 0,
                        bytes: bytes_1.bs58.encode(new anchor_1.BorshAccountsCoder(this.program.idl).accountDiscriminator(types_1.AccountType.MarginfiAccount)),
                    },
                },
            ],
        })).map((a) => a.pubkey);
     ...
     */
    async createMarginfiGroup(opts) {
        const dbg = require("debug")("mfi:client");
        const accountKeypair = web3_js_1.Keypair.generate();
     ...
     */
    async createLendingPool(bankMint, bankConfig, opts) {
        const dbg = require("debug")("mfi:client");
        const bankKeypair = web3_js_1.Keypair.generate();
        const ixs = await this.group.makePoolAddBankIx(this.program, this.provider.connection, bankKeypair.publicKey, bankMint, bankConfig);
        const signers = [...ixs.keys, bankKeypair];
        const priorityFeeIx = (0, utils_1.makePriorityFeeIx)(0.001);
        ...
        try {
            ...
            if (error.logs) {
                ...
                if (errorParsed) {
                    ...
                    throw new errors_1.ProcessTransactionError(errorParsed.description, errors_1.ProcessTransactionErrorType.SimulationError, error.logs);
                }
            }
    ...
    async simulateTransaction(transaction, accountsToInspect) {
        ...
        if (error.logs) {
            ...
            if (errorParsed) {
                console.log("Parsed:", errorParsed);
                throw new errors_1.ProcessTransactionError(errorParsed.description, errors_1.ProcessTransactionErrorType.SimulationError, error.logs, errorParsed.programId);
            }
        }
        console.log("fallthrough error", error);
        throw new errors_1.ProcessTransactionError("Something went wrong", errors_1.ProcessTransactionErrorType.FallthroughError);
